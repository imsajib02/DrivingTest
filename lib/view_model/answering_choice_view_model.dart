...
class AnsweringChoiceViewModel extends ChangeNotifier implements BasicViewModel {
  ...
  Future<void> getQuestionAvailability(BuildContext context, List<bool> categorySelection) async {
        ...
        if(questionAvailability.isAvailable) {
          ...
          switch (questionTypeID) {
            ...
            case 2:
              ...
              !isValid ? infoMessage = AppLocalization.of(context).getTranslatedValue("no_incorrectly_answered_questions_info") : "";
              break;

            case 3:
              isValid = await AnsweringChoiceRepository().isChoiceValid(15, 0, categorySelection);
              !isValid ? infoMessage = AppLocalization.of(context).getTranslatedValue("no_unseen_left_info") : "";
              break;

            case 4:
              isValid = await AnsweringChoiceRepository().isChoiceValid(18, 1, categorySelection);
              !isValid ? infoMessage = AppLocalization.of(context).getTranslatedValue("no_favourite_questions_info") : "";
              break;
          }

          !isValid ? topTitle = AppLocalization.of(context).getTranslatedValue("question_shuffle") : "";
        }
        else {

          topTitle = AppLocalization.of(context).getTranslatedValue("practise_info");

          infoMessage = AppLocalization.of(context).getTranslatedValue("not_enough_questions_info1") + questionAvailability.totalAmount.toString() +
              AppLocalization.of(context).getTranslatedValue("not_enough_questions_info2") + questionAvailability.choiceAmount.toString() +
    ...
    if(!isDisposed) {
      ...
      if(questionAvailability.isAvailable) {
        ...
        if(isValid) {
          ...
          shouldShow ? setIndex(infoViewIndex) : answeringChoiceInterface.onStartPress();
        }
      }
      else {
